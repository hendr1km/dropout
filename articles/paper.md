...
# Installation
The dropout package can be installed using CRAN, as well as GitHub for access to the development version. For the second type of installation devtools is needed.
```r
# CRAN
...
For the following examples we will use an adapted version of the Flying Etiquette by five-thirty-eight dataset that is included in the `dropout` package. In these workflow examples, I will be using dplyr verbs [@dplyr] - although this is not necessary.
